...
def unique_entities(n_samples=10, sample_size=10):
    """Simulates a collection of n samples of size sample_size where no entity
    is observed more than once, calculates the BBC estimate of the population
    size, and calculates the uncorrected Cuthbert estimate after repeating one
    entity to ensure convergence.
    ...
    # Append entity '0' to last sample to ensure Cuthbert convergence; calculate
...
def random_samples(pop_size, sample_sizes, n_samples=None,):
    ...
    results['bbc'] = est.bbc(samples, max_delta)
    results['cuthbert'] = est.cuthbert(samples, min_survival, cv, cv_ppn)
    return results
